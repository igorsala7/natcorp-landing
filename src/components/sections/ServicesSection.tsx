import { ClipboardCheck, Code2, GraduationCap, Handshake, Headset, Users } from 'lucide-react'
...
/** Serviços que acompanham o sistema: implantação, BPO, treinamento, fábrica, projeto e suporte. */
        ...
        <div>
          ...
          <Stagger className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" stagger={0.08}>
            {services.map((sv) => (
              <StaggerItem
                key={sv.title}
                className="group flex gap-4 rounded-2xl border border-brand-mist bg-white p-5 transition-[transform,box-shadow,border-color,background-color] duration-500 ease-brand hover:-translate-y-1 hover:border-brand-purple/30 hover:bg-white hover:shadow-lift"
              >
